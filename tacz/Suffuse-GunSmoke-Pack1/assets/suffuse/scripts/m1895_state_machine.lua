...
local function runInspectAnimation(context)
    ...
    elseif (context:getAmmoCount() <= 0 and context:hasBulletInBarrel()) then
        context:runAnimation("inspect_left1", track, false, PLAY_ONCE_STOP, 0.2)
    else
...
function reload_state.entry(this, context)
    ...
    if (isNoAmmo) then
        reload_state.init_state = 1
    else
        reload_state.init_state = 0
    end
    context:runAnimation("reload_tactical_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
...
function reload_state.update(this, context)
    ...
    if (state.loaded_ammo > state.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if (context:isHolding(track)) then
            context:runAnimation("reload_tactical_loop", track, false, PLAY_ONCE_HOLD, 0)
...
function reload_state.transition(this, context, input)
    if (input == this.INPUT_RELOAD_RETREAT or input == INPUT_CANCEL_RELOAD) then
        if (reload_state.init_state == 1) then
            context:runAnimation("reload_empty_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("reload_tactical_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
