...
local function runPutAwayAnimation(context)
    ...
    if(context:getFireMode() == BURST)then
        if ((not context:hasBulletInBarrel()) and context:getAmmoCount() == 0) then
            context:runAnimation("put_away_semi_empty", track, false, PLAY_ONCE_HOLD, put_away_time * 0.75)
        else
...
function start_state.transition(this, context, input)
    if (input == INPUT_DRAW) then
        ...
        if(context:getFireMode() == BURST)then
            if ((not context:hasBulletInBarrel()) and context:getAmmoCount() == 0) then
                context:runAnimation("draw_semi_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
            else
...
function reload_state.entry(this, context)
    ...
    if (isNoAmmo) then
        ...
        if (context:getFireMode() == BURST) then
            context:runAnimation("reload_intro_empty_semi", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
        else
