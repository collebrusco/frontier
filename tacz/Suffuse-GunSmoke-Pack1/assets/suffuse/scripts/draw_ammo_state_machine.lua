...
function bolt_caught_states.normal.update(this, context)
    ...
    if (isNoAmmo(context)) then
        ...
        if (a < 3) then
            context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),0.1+(2-a)*0.5,false)
        else
...
function main_track_states.start.transition(this, context, input)
    ...
    if (input == INPUT_DRAW) then
        if (this.main_track_states.draw_counter == 0) then
            context:runAnimation("first_draw", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
            this.main_track_states.draw_counter = 1
        elseif (this.main_track_states.draw_counter ~= 0) then
