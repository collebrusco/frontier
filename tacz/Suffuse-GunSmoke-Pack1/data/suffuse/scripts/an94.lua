...
local function getReloadTimingFromParam(param)
    ...
    -- 这四个是扩容后的那四个参数
    local tactical_feed_extended = param.tactical_feed_extended * 1000
    local tactical_cooldown_extended = param.tactical_cooldown_extended * 1000
    local empty_feed_extended = param.empty_feed_extended * 1000
    local empty_cooldown_extended = param.empty_cooldown_extended * 1000
    -- 这两个判断是用来检查以上八个参数是否有缺失的，若有缺失则不获取任何参数。其实是可以写进一个判断语句的，但是这样的话整个句子会过长影响阅读所以我就拆成两个了
    if (tactical_feed == nil or tactical_cooldown == nil or empty_feed == nil or empty_cooldown == nil) then
        return nil
    end
    if (tactical_feed_extended == nil or tactical_cooldown_extended == nil or empty_feed_extended == nil or empty_cooldown_extended == nil) then
        return nil
    end
    -- 顺序返回获取到的这八个参数
    return tactical_feed, tactical_cooldown, empty_feed, empty_cooldown, tactical_feed_extended, tactical_cooldown_extended, empty_feed_extended, empty_cooldown_extended
end
...
function M.tick_reload(api)
    ...
    local tactical_feed, tactical_cooldown, empty_feed, empty_cooldown, tactical_feed_extended, tactical_cooldown_extended, empty_feed_extended, empty_cooldown_extended = getReloadTimingFromParam(param)
    ...
    -- 判断此时玩家手里拿的枪是不是 0 或 1 级扩容
    if (api:getMagExtentLevel() == 0 or api:getMagExtentLevel() == 1) then
        -- 0 级和 1 级扩容的战术换弹
    ...
    -- 判断此时玩家手里拿的枪是不是 2 或 3 级扩容
    -- 由于内容和上文几乎完全一样，只改了几个时间参数的名字，所以这部分的注释就不写了
    elseif ((api:getMagExtentLevel() == 2 or api:getMagExtentLevel() == 3)) then
        if (cache.is_tactical) then
            if (reload_time < tactical_feed_extended) then
                return TACTICAL_RELOAD_FEEDING, tactical_feed_extended - reload_time
            elseif (reload_time >= tactical_feed_extended and reload_time < tactical_cooldown_extended) then
                if (cache.reloaded ~= 1) then
                    if (api:isReloadingNeedConsumeAmmo()) then
                        api:putAmmoInMagazine(api:consumeAmmoFromPlayer(cache.needed_count))
                    else
                ...
                return TACTICAL_RELOAD_FINISHING, tactical_cooldown_extended - reload_time
            else
                return NOT_RELOADING, -1
            end

        else
            if (reload_time < empty_feed_extended) then
                return EMPTY_RELOAD_FEEDING, empty_feed_extended - reload_time
            elseif (reload_time >= empty_feed_extended and reload_time < empty_cooldown_extended) then
                if (cache.reloaded ~= 1) then
                    if (api:isReloadingNeedConsumeAmmo()) then
                        api:putAmmoInMagazine(api:consumeAmmoFromPlayer(cache.needed_count) - 1)
                    else
                ...
                return EMPTY_RELOAD_FINISHING, empty_cooldown_extended - reload_time
            else
                return NOT_RELOADING, -1
            end
        end
